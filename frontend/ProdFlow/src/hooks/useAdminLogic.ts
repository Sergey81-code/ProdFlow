...
export const useAdminLogic = () => {
  ...
  const handleUserSubmit = async (vals: any) => {
    try {
      if (editingUser?.id) {
        await usersQuery.update.mutateAsync({ id: editingUser.id, payload: vals });
      } else {
  ...
  const handleRoleSubmit = async (vals: any) => {
    try {
      if (editingRole?.id) {
        await rolesQuery.update.mutateAsync({ id: editingRole.id, payload: vals });
      } else {
  ...
  const handleDeviceSubmit = async (vals: any) => {
    try {
      if (editingDevice?.id) {
        await devicesQuery.update.mutateAsync({ id: editingDevice.id, payload: vals });
      } else {
