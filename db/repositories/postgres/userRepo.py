...
class PostgresUserRepo(IUserRepository):
    ...
    async def update(self, user: User, info: UpdateUser) -> User:
        ...
        if user_data:
            ...
            updated_user = await self._session.get(UserDb, user.id)
        ...
        await self._session.refresh(updated_user, attribute_names=["roles"])
        return User.model_validate(updated_user)

    async def delete(self, id: UUID) -> UUID:
        stmt_roles = delete(UserRole).where(UserRole.user_id == id)
        await self._session.execute(stmt_roles)

        stmt_user = delete(UserDb).where(UserDb.id == id).returning(UserDb.id)
        result = await self._session.execute(stmt_user)

        await self._session.commit()

        deleted_id = result.unique().scalar_one_or_none()
        return deleted_id
    ...
    async def get_user_permissions(self, user_id: UUID) -> list[str]:
        stmt = (
            ...
            .select_from(UserDb)
            .join(UserRole, UserRole.user_id == UserDb.id)
            .join(Role, Role.id == UserRole.role_id)
            .where(UserDb.id == user_id)
        )
        result = await self._session.execute(stmt)
        return list(set(result.unique().scalars().all()))
