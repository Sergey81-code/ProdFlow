...
async def test_get_device_by_android_id(client, create_device_in_database, get_device_from_database):
    device_id = uuid4()
    ...
    resp = client.get(f"{VERSION_URL}{DEVICE_URL}/android/{android_id}", headers=headers)
...
async def test_get_device_by_android_id_not_found(client):
    ...
    assert resp.status_code == 404
    assert resp.json() == {"detail": f"Device with this android id {android_id} not found"}


async def test_get_device_by_android_id_unauthorized(client, create_device_in_database, get_device_from_database):
    device_id = uuid4()
    android_id = "a3f9c2b7d18e44fa"
...
async def test_get_device_by_android_id_invalid(
    ...
    await create_device_in_database({"id": device_id, "name": "some device", "android_id": "valid_android_id"})
